...
router.get('/', async function(req, res, next) {
    var query = "SELECT i.invoice_id, export_date, employee_id, p.product_name, p.price, sl.quantity, sl.cust_phone from Serve_line sl join Product p on sl.product_id = p.product_id join Invoice i on sl.invoice_id = i.invoice_id where i.invoice_id = (SELECT max(invoice_id) from Serve_line)";
